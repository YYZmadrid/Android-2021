...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                List<Message> repos = baseGetReposFromRemote();
                if (repos != null && !repos.isEmpty()) {
                    new Handler(getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            if(studentId == null)  adapter.setData(repos);
                            else{
                                List <Message> my_repos = new ArrayList<>();
                                adapter.setData(my_repos);
                                for(Message s : repos){
                                    if(s.getStudentId().equals(studentId)) my_repos.add(s);
                                }
                                if(!my_repos.isEmpty()) adapter.setData(my_repos);
                            }
                        }
                    });
    ...
    public List<Message> baseGetReposFromRemote() {
        String urlStr =
